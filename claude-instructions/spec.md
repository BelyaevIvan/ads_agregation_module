...
Методы возвращают мета-информацию для динамического построения UI-фильтров на фронте. Данные агрегируются из активных объявлений и кэшируются в сервисе на 5 минут, чтобы не нагружать БД.

---

### `GET /filters/sizes` 🌐
**Получить все уникальные размеры, встречающиеся в активных объявлениях.**

Используется на фронте для построения чек-боксов в секции «Размер» панели фильтров — список не зашит в код, а всегда актуален относительно состояния БД.
...
Каждый массив отсортирован численно по возрастанию. Нечисловые значения (если вдруг попали из LLM-парсера) отправляются в конец и сортируются лексикографически.

#### Алгоритм
1. Если в in-memory кэше лежит свежий результат (младше 5 минут) — вернуть его. Иначе — продолжить.
2. Параллельно выполнить три запроса к `listings`:
```sql
SELECT DISTINCT unnest(size_rus) FROM listings WHERE is_hidden = FALSE AND size_rus IS NOT NULL
-- аналогично для size_eu, size_us
```
3. Для каждой из трёх выборок:
   - Удалить пустые значения.
   - Отсортировать численно: `strconv.ParseFloat` для каждого элемента, сравнение чисел. Невалидные (не распарсились как float) — в конец, между собой по алфавиту.
4. Собрать результат в структуру `{size_rus, size_eu, size_us}`.
5. Положить в кэш с `expiresAt = now() + 5 минут`.
6. Вернуть.
...
| GET | `/api/v1/filters/sizes` | 🌐 | Уникальные размеры из активных объявлений |
